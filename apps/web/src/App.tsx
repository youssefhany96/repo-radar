...
import {
  ...
  IconButton, Badge, CssBaseline, ThemeProvider, createTheme, useMediaQuery,
} from "@mui/material";
    ...
    <ThemeProvider theme={theme}>
      ...
      <AppBar position="sticky" color="default" elevation={0}
        ...
        <Toolbar>
          ...
          <Typography variant="h6" sx={{ flex: 1, fontWeight: 600 }}>Repo Radar</Typography>
          <IconButton
      ...
      <Container maxWidth="md" sx={{ py: 3 }}>
        <Tabs value={tab} onChange={(_, v: number) => setTab(v)} sx={{ mb: 3 }}>
          ...
          <Tab
            label={
              <Badge badgeContent={trackedCount} color="primary"
        ...
        <Box role="tabpanel" hidden={tab !== 0}>{tab === 0 && <SearchPanel />}</Box>
        <Box role="tabpanel" hidden={tab !== 1}>{tab === 1 && <TrackedPanel />}</Box>
      </Container>
    </ThemeProvider>
  );
}
